...
void sonic_write_gc_profile(gzFile sonic_file, FILE *ref_file, int number_of_chromosomes, char **chromosome_names)
{
	...
	int written_gc;
	int end_of_gc;
	char end_of_chr = (char) SONIC_END_OF_CHR ;
	...
	while (!feof(ref_file)){
		...
		if (ch == '>'){
			...
			if( written_gc != 0)
			{
				gzwrite(sonic_file, &end_of_chr, sizeof(gc_content));
			}
...
int sonic_find_chromosome_index(char **chromosome_names, const char *this_chromosome, int number_of_chromosomes){
	int i;
...
int sonic_refind_chromosome_index(sonic *this_sonic, const char *this_chromosome){
	int i;
